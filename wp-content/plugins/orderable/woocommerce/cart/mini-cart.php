...
<?php if ( isset( WC()->cart ) && ! WC()->cart->is_empty() ) : ?>
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<li class="orderable-mini-cart-item <?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'mini_cart_item', $cart_item, $cart_item_key ) ); ?>">
						<?php if ( Orderable_Helpers::is_variation_product( $_product->get_type() ) || ! empty( $cart_item['orderable_fields'] ) ) : ?>
							<button
						...
						<?php Orderable_Products::get_quantity_roller( $cart_item, $product_price ); ?>
					</li>
					<?php
				}
			}
	...
	<div class="orderable-mini-cart__notices"></div>
