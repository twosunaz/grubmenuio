( function ( $, document ) {
	const classes = {
		...
		optionsButtonIsOpen:
			'orderable-receipt-layouts__receipt-layout-options-button--is-open',
		...
		optionPrintLink:
			'orderable-receipt-layouts__receipt-layout-option-print-link',
		optionPrintButton:
			'orderable-receipt-layouts__receipt-layout-option-print-button',
		optionPrintButtonIsLoading:
			'orderable-receipt-layouts__receipt-layout-option-print-button--is-loading',
		loadingOptionPrintButton:
			'orderable-receipt-layouts__receipt-layout-option-loading',
		loadingOptionPrintButtonIsActive:
	...
	function getReceiptURL( orderId, receiptLayoutId ) {
		const layoutId =
			receiptLayoutId || orderableReceiptLayouts.receiptLayoutId;
	...
	function onOptionPrintButtonClick( event ) {
		...
		const $loading = $( this ).siblings(
			`.${ classes.loadingOptionPrintButton }`
		);

		const orderId = $optionPrintButton.attr( 'data-order-id' );
		const receiptLayoutId = $optionPrintButton.attr(
			'data-receipt-layout-id'
		);

		const setLoading = ( value ) => {
			if ( value ) {
				$optionPrintButton.prop( 'disabled', true );
				$optionPrintButton.addClass(
					classes.optionPrintButtonIsLoading
				);
				$loading.addClass( classes.loadingOptionPrintButtonIsActive );
			} else {
				$optionPrintButton.prop( 'disabled', false );
				$optionPrintButton.removeClass(
					classes.optionPrintButtonIsLoading
				);
				$loading.removeClass(
					classes.loadingOptionPrintButtonIsActive
				);
			}
		};
		...
		getReceiptURL( orderId, receiptLayoutId )
			.then( openInNewTab )
			.finally( () => {
				setLoading( false );
				closeList();
	...
	function closeList( $optionButton ) {
		...
		$optionButton
			.siblings( `.${ classes.optionsList }` )
			.removeClass( classes.optionsListIsOpen );
	}
	...
	function openList( $optionButton ) {
		...
		$optionButton
			.siblings( `.${ classes.optionsList }` )
			.addClass( classes.optionsListIsOpen );
	}
	...
	$( document ).ready( function () {
		...
		$( document.body ).on(
			'click',
			`.${ classes.optionPrintLink }`,
			onOptionPrintLinkClick
		);
		$( document.body ).on(
			'click',
			`.${ classes.optionPrintButton }`,
			onOptionPrintButtonClick
		);
		$( document.body ).on(
			'click',
