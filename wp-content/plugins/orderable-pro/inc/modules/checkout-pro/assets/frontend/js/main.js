(function ($, document) {
  ...
  var orderable_checkout_pro = {
     ...
     */
    on_ready() {
      $(document.body).on('click', '.orderable-checkout-summary-toggle', orderable_checkout_pro.hide_show_order_summary);
      ...
      if (orderable_checkout_pro_params) {
         ...
         */
        $('.orderable-checkout__form').on('submit', 'form.checkout_coupon', orderable_checkout_pro.on_submit_coupons_form);
      }
    },
    update_total() {
    ...
    mobile_view() {
      ...
      if (width <= 991) {
        var shipping_table = $('#order_review .orderable-checkout__shipping-table');
        const tip_form = $('#order_review .orderable-tip');
        const coupon_form = $('#order_review .coupon-form');
        if (shipping_table.length) {
          const additional_fields = $('.woocommerce-additional-fields');
          const title = $('<h3></h3>').text(orderable_checkout_pro_params.i18n.shipping_title);
          title.addClass('orderable_shipping_title');
          $('#customer_details .orderable-checkout__shipping-table').remove();
          $('#customer_details .orderable_shipping_title').remove();
          if (additional_fields.length) {
            title.insertBefore(additional_fields);
            $('#order_review .orderable-checkout__shipping-table').insertBefore(additional_fields);
          } else {
            $('#customer_details').prepend($('#order_review .orderable-checkout__shipping-table'));
        ...
        if (tip_form.length) {
          ...
          $('#payment table.orderable-mobile-table').prepend($('#order_review .orderable-tip-row'));
        }
        if (coupon_form.length) {
          $('#payment .coupon-form').remove();
          $('#payment table.orderable-mobile-table').prepend($('#order_review .coupon-form'));
        }
      } else {
        ...
        $('#order_review .orderable-checkout__order-review thead > tr > td').prepend($('.orderable-checkout__shipping-table'));
        $('#order_review .woocommerce-checkout-review-order-table > tbody').append($('#payment .coupon-form'));
        $('#order_review .woocommerce-checkout-review-order-table > tbody').append($('#payment .orderable-tip-row'));
        if (payment_table.length) {
          payment_table.remove();
        }
      }
    ...
    hide_show_order_summary() {
      if ($('.orderable-checkout-summary-toggle_link--hide').is(':visible')) {
     ...
     */
    set_date_and_time() {
      if (wp.hooks.applyFilters('orderable_pro_timing_disable_auto_select_date_time', false)) {
        return;
      }
    ...
    toggle_delivery_area() {
      const shipping_method = document.querySelector('input.shipping_method:checked') || document.querySelector('input.shipping_method');
      const shipping_title = document.querySelector('.orderable-checkout-section--shipping h3');
      const shipping_fields = document.querySelector('.orderable-checkout-section--shipping .woocommerce-shipping-fields');
      const shipping_checkbox = document.getElementById('ship-to-different-address-checkbox');
      const shipping_wrapper = document.querySelector('.shipping_address');
      let value = [];
      ...
      if (shipping_title.style.display === 'none' && shipping_fields.style.display !== 'none') {
      ...
      if (!shipping_checkbox && 'shipping' !== orderable_checkout_pro_params.woocommerce_ship_to_destination && value.includes('local_pickup:')) {
