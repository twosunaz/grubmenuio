...
class ErrorLog
{
  ...
  {
    add_action("rest_api_init", ["SnipVault\Rest\ErrorLog", "register_custom_endpoint"]);
  }
  ...
  {
    register_rest_route("snipvault/v1", "/errorlog", [
      "methods" => "GET",
      "callback" => ["SnipVault\Rest\ErrorLog", "get_error_items"],
  ...
  public static function get_log_directory()
  {
    ...
    if (is_wp_error($logdir)) {
      ...
      $returndata["message"] = __("Log file does not exist", "snipvault");
    ...
    if (is_string($logdir) && !empty($logdir)) {
      ...
      if (file_exists($logdir)) {
        ...
        $returndata["error"] = true;
        $returndata["message"] = __("Log file does not exist", "snipvault");
        $returndata["description"] = __("The specified PHP error log path does not exist. Path: ", "snipvault") . $logdir;
        return new \WP_Error("log_file_not_found", $returndata["message"], ["status" => 404]);
      }
    }
    ...
    if ($logdir === "syslog") {
      ...
      $returndata["error"] = true;
      $returndata["message"] = __("Errors are logged to the system log", "snipvault");
      $returndata["description"] = __("The PHP error_log directive is set to 'syslog'. Please check your system log for errors.", "snipvault");
    ...
    if ($logdir === "stderr") {
      ...
      $returndata["error"] = true;
      $returndata["message"] = __("Errors are sent to the standard error output", "snipvault");
      $returndata["description"] = __("The PHP error_log directive is set to 'stderr'. Errors are not logged to a file.", "snipvault");
      return new \WP_Error("stderr_logging", $returndata["message"], ["status" => 400]);
    }

    // Unable to determine the error log directory
    $returndata["error"] = true;
    $returndata["message"] = __("Unable to determine the error log directory", "snipvault");
    $returndata["description"] = __("The PHP error_log directive is not set or has an unsupported value.", "snipvault");
  ...
  private static function parse($logFilePath)
  {
    ...
    foreach ($lines as $currentLine) {
      ...
      if ("[" === $currentLine[0] && !preg_match("/Stack trace/", $currentLine) && !preg_match("/.*PHP.*[0-9]\. /", $currentLine)) {
        ...
        $uniqueId = uniqid("snipvault_", true);
